/*
 * core4web.c - a minimal HTTP server that serves static and
...
void cerror(FILE *stream, char *cause, char *errno,
  ...
  fprintf(stream, "\n");
  fprintf(stream, "<html><title>Core4web</title>");
  fprintf(stream, "<body bgcolor=""ffffff"">\n");
  fprintf(stream, "%s: %s\n", errno, shortmsg);
  fprintf(stream, "<p>%s: %s\n", longmsg, cause);
  fprintf(stream, "<hr><em>Cor4web server 1.0 by: Divine-IT Limited</em>\n");
}
...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    /* Core4web only supports the GET method */
    if (strcasecmp(method, "GET")) {
      cerror(stream, method, "501", "Not Implemented",
	     "Cor4web does not implement this method");
      fclose(stream);
      close(childfd);
      continue;
    }
	...
	*p = '\0';
    ...
    if (stat(filename, &sbuf) < 0) {
      cerror(stream, filename, "404", "Not found",
	     "core4web couldn't find this file");
      fclose(stream);
      close(childfd);
      continue;
    }
	...
	strcpy(filetype, "text/plain");
      ...
      fprintf(stream, "Server: Cor4web Server\n");
	...
	continue;
      ...
      sprintf(buf, "Server: Cor4web Server\n");
      write(childfd, buf, strlen(buf));
